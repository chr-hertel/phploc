...
The two formatters differ on purpose: the text report omits whole sections when there are no classes or functions, while the JSON output always emits every key so that consumers can rely on a stable shape. `--json` also suppresses the version banner, since it would make the output invalid JSON.
