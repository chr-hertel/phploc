...
- **Namespaces are flat**: everything is `SebastianBergmann\PHPLOC`, including classes under `src/Exception/`. Directories are organizational only — autoloading is a `classmap` over `src/`, so directory names carry no namespace meaning.
